...
export const solutionPartTwo = (input: string): number => {
    ...
    const orderedDirSizes = pipe(
        ...
        E.chain((dirsMap) => E.of(Array.from(dirsMap.values()))),
        E.chain((dirValues) => E.of(dirValues.sort((a, b) => a - b))),
        E.getOrElse(() => [] as number[])
    )

    const currSpaceTaken = orderedDirSizes[orderedDirSizes.length - 1]
    const spaceAvailable = 70000000 - currSpaceTaken
    for (const dirSize of orderedDirSizes) {
        if (spaceAvailable + dirSize > 30000000) {
            return dirSize
        }
    }
